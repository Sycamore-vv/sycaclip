...
let suppressSubInputFocusUntil = 0;
...
function focusSubInputWithRetry(attempt = 0) {
    if (Date.now() < suppressSubInputFocusUntil) {
        return;
    }
    ...
    try {
        ...
        console.error('sub input focus failed:', error);
    }
    ...
    setTimeout(() => {
        focusSubInputWithRetry(attempt + 1);
    }, 80 * (attempt + 1));
}
...
function isInteractiveElement(target) {
    ...
    return !!target.closest(
        'input, textarea, select, button, [contenteditable="true"], [contenteditable=""], a, .content-text, .dialog-content, .dialog-input, .context-menu'
    );
}
...
document.addEventListener('pointerdown', (event) => {
    if (isInteractiveElement(event.target)) {
        suppressSubInputFocusUntil = Date.now() + 3000;
    }
}, true);
...
document.addEventListener('selectionchange', () => {
    ...
    if (anchorElement?.closest('.content-text, textarea, input, [contenteditable="true"], [contenteditable=""], .dialog-content')) {
        suppressSubInputFocusUntil = Date.now() + 3000;
    }
});

window.addEventListener('DOMContentLoaded', () => {
    if (window.ztools?.clipboard) {
        window.ztools.clipboard.updateText = (id, text) =>
            electron.ipcRenderer.invoke('clipboard:update-text', id, text);
    }
