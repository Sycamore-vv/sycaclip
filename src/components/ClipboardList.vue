...
defineProps({
  ...
  groupAssignments: { type: Object, default: () => ({}) },
  ...
  <div class="clipboard-list" @scroll="emit('scroll', $event)">
    ...
    >
      <TextItem
        ...
        :group-name="groups.find(group => group.id === groupAssignments[item.id])?.name || ''"
...
.clipboard-item.selected {
  border: 1px solid rgb(26 115 232 / 55%);
  background: rgb(26 115 232 / 10%);
  border-radius: 5px;
}

.clipboard-item.multi-selected {
  background: rgb(26 115 232 / 14%);
  border: 1px solid rgb(26 115 232 / 32%);
  border-radius: 5px;
}
